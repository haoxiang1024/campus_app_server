...
public interface LostFoundMapper {
    ...
    //添加丢失物品信息
    boolean addLost(@Param("lostFound") LostFound lostFound);

    //根据用户id获取用户发布的信息
    List<LostFound> getAllByIdLostList(@Param("user_id") int user_id);

    //修改状态
    boolean updateState(@Param("id")int id,@Param("state")String state);

    //显示置顶信息
    List<LostFound>showLostList(@Param("stick")int stick);
    //添加数据用的
    //更新失物图片
    ...
    List<LostFound> selectAllLost(@Param("keyword") String keyword);
    // 根据ID删除
    ...
    List<LostFound> selectLostByPage();
}
